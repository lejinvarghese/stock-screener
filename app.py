...
def trigger_otp():
    ...
    try:
        ...
        return jsonify({'success': True, 'message': 'OTP request sent! Check your authenticator app.'})
    except Exception as e:
...
def recommend_stocks():
    ...
    try:
        ...
        console.print(Panel(f"[bold cyan]Pre-selected stocks:[/bold cyan] {', '.join(pre_selected_stocks)}", title="Analysis Results", border_style="cyan"))
        optimized_stocks = optimize(pre_selected_stocks, value=1000)
        console.print(Panel(f"[bold green]Portfolio optimization completed![/bold green]", title="Success", border_style="green"))
        ...
        if os.path.exists(f"{image_dir}/pf_cov_matrix.png"):
            images['covariance_matrix'] = f"/images/pf_cov_matrix.png"
        ...
        for stock in pre_selected_stocks:
            ...
            if os.path.exists(chart_path):
                stock_charts.append({
                    'symbol': stock,
                    'chart': f"/images/ohlc_{stock}.png"
                })
...
def import_watchlist():
    ...
    try:
        ...
        if added_count == 0 and skipped_count == 0:
            return jsonify({'error': 'No symbols could be imported. Check CSV format.'}), 400
...
if __name__ == "__main__":
    ...
    console.print(Panel(f"[bold green]Stock Screener Flask App Starting![/bold green]\n[blue]Server running on: http://0.0.0.0:5004[/blue]\n[yellow]Ready to analyze portfolios![/yellow]", title="Stock Screener", border_style="green"))
