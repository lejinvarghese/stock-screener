...
def get_data(ticker, period=36):
    ...
    """
    console.print(f"[bold cyan]Fetching data for:[/bold cyan] [yellow]{ticker}[/yellow]")
    ...
    try:
        ...
        data["ticker"] = ticker
        console.print(f"[green]Successfully fetched {len(data)} rows for {ticker}[/green]")
        return data
        ...
        try:
            ...
            if not data.empty:
                data["ticker"] = ticker
                console.print(f"[green]Successfully fetched {len(data)} rows for {ticker} via yfinance[/green]")
                return data
            else:
                console.print(f"[yellow]WARNING: No data returned from yfinance for {ticker}[/yellow]")
                return None
