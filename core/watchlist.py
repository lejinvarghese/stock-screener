...
class WatchlistManager:
    ...
    def add_symbol(self, watchlist_name: str, symbol: str) -> bool:
        ...
        try:
            ...
            console.print(
                f"[yellow]{symbol} already exists in {watchlist_name}[/yellow]"
            )
            return False
    ...
    def remove_symbol(self, watchlist_name: str, symbol: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if cursor.rowcount > 0:
                ...
                console.print(
                    f"[yellow]{symbol} not found in {watchlist_name}[/yellow]"
                )
                return False
    ...
    def import_from_csv(self, watchlist_name: str, csv_content: str) -> Tuple[int, int]:
        ...
        try:
            ...
            if not symbol_column:
                console.print(
                    f"[red]No symbol column found. Available columns: {csv_reader.fieldnames}[/red]"
                )
                return 0, 0
        ...
        console.print(
            f"[green]Import complete: {added_count} added, {skipped_count} skipped[/green]"
        )
...
def get_custom_watchlist(watchlist_name: str = "Default") -> List[str]:
    ...
    if not symbols:
        ...
        console.print(
            f"[yellow]No symbols in {watchlist_name}, using default stocks[/yellow]"
        )
...
if __name__ == "__main__":
    ...
    console.print(
        Panel("[bold blue]Testing Watchlist Manager[/bold blue]", border_style="blue")
    )
    ...
    console.print(
        Panel(
            f"[bold green]Default watchlist symbols:[/bold green] {symbols}",
            border_style="green",
        )
    )
    ...
    console.print(
        Panel(f"[bold cyan]CSV Export:[/bold cyan]\n{csv_content}", border_style="cyan")
    )
