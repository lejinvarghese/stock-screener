...
def trigger_otp_request():
    ...
    try:
        ...
        console.print(f"[cyan]Attempting to initialize Wsimple with username:[/cyan] [yellow]{username}[/yellow]")
        ...
        if "'Wsimple' object has no attribute 'box'" in str(e):
            ...
            console.print("[yellow]WARNING: This might be a version compatibility issue with wsimple library[/yellow]")
            # Try to continue anyway
            otp_requested = False
            raise Exception("Wsimple library compatibility issue. Try updating the library.")
        else:
            otp_requested = False
            raise e
            ...
            try:
                ...
                if isinstance(watchlist, dict):
                    if "securities" in watchlist:
                        ...
                        tickers = [ticker["stock"]["symbol"] for ticker in watchlist["results"]]
                    else:
                        console.print(f"[yellow]WARNING: Unexpected watchlist format[/yellow]")
                        return []
                else:
                    console.print(f"[yellow]WARNING: Watchlist is not a dict: {type(watchlist)}[/yellow]")
                    return []
                    
                if tickers:
                    console.print(f"[green]Found {len(tickers)} tickers in watchlist[/green]")
                    console.print(f"[blue]Sample ticker: {tickers[0]}[/blue]")
                else:
                    console.print("[yellow]WARNING: No tickers found in watchlist[/yellow]")
                return tickers
            except AttributeError as e:
                if "'Wsimple' object has no attribute 'box'" in str(e):
                    console.print(f"[red]ERROR: Known wsimple library bug with 'box' attribute: {e}[/red]")
                    console.print("[yellow]WARNING: This is a compatibility issue with wsimple 3.0.1 and Python 3.13[/yellow]")
                    console.print("[yellow]Returning empty list to use fallback stocks[/yellow]")
                    return []
                else:
                    raise e
        else:
...
if __name__ == "__main__":
    ...
    console.print(Panel(f"[bold green]Sample watchlist:[/bold green] {watchlist}", title="Wealthsimple", border_style="green"))
