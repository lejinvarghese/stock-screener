...
def run_engine(n_tickers = 15):
    ...
    with Pool(cores) as p:
        selected_stocks = p.map(analyze_ticker, watchlist)

    selected_stocks = list(filter(None.__ne__, selected_stocks))
    send_text(TELEGRAM_TOKEN, TELEGRAM_ID, f'It is only logical to consider these {len(selected_stocks)} stocks: ' + '; '.join(selected_stocks))
