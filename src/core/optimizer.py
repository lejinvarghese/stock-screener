...
def optimize(prices, value = 1000):
    ...
    try:
        plotting.plot_efficient_frontier(ef, ef_param='return', show_assets=True, filename = f'{PATH}/data/outputs/pf_optimizer.png')
        plt.close()
    except:
        pass
    ...
    plotting.plot_covariance(risk_models.sample_cov(prices, log_returns=True), filename = f'{PATH}/data/outputs/pf_cov_matrix.png')
    plt.close()

    latest_prices = discrete_allocation.get_latest_prices(prices)
    allocation, leftover = discrete_allocation.DiscreteAllocation(cleaned_weights, latest_prices, total_portfolio_value=value).lp_portfolio()
    print(f"Recommended Allocation: {allocation}")
    print("Funds remaining: ${:.2f}".format(leftover))


    send_image(TELEGRAM_TOKEN, TELEGRAM_ID,
                   f'{PATH}/data/outputs/pf_optimizer.png')
    send_image(TELEGRAM_TOKEN, TELEGRAM_ID,
                f'{PATH}/data/outputs/pf_cov_matrix.png')
    send_message(TELEGRAM_TOKEN, TELEGRAM_ID,
        f"Recommended Allocation: A portfolio {len(allocation)} stocks: {allocation}")
    send_message(TELEGRAM_TOKEN, TELEGRAM_ID, "Performance: Expected Annual Return: {:.2%}, Volatility: {:.2%}, Sharpe Ratio: {:.2f}".format(*list(ef.portfolio_performance())))
